...
test2('Multiple valid orders are accepted into the limit order book Given an empty orderbook for "TW"', () => {
  ...
  let orderBook = Clob.getLiveOrders('TW')
...
test3('Two tradable orders result in a trade Given an empty orderbook for "TW"', () => {
  ...
  assert.equal(parsedResponse2.trades[0].buyOrder.trader.username, 'trader1')
  assert.equal(parsedResponse2.trades[0].sellOrder.trader.username, 'trader2')
})
...
test4('Two tradable orders with different quantities are partially filled Given an empty orderbook for "TW"',
  () => {
    ...
    let { order, trades } = JSON.parse(response2)
    assert.equal(trades[0].ticker, 'TW')
    assert.equal(trades[0].price, 9950)
    assert.equal(trades[0].quantity, 100)
    assert.equal(trades[0].buyOrder.trader.username, 'trader1')
    assert.equal(trades[0].sellOrder.trader.username, 'trader2')
  }
)
